...
from Data.FuncionesMineria import mosaico_targetbinaria, graficoVcramer, boxplot_targetbinaria, hist_targetbinaria, \
    Transf_Auto, Vcramer, lm, Rsq, crear_data_modelo, modelEffectSizes, validacion_cruzada_lm
...
mosaico_targetbinaria(df_input['ActividadPpal'], varObjBin1, 'ActividadPpal')
mosaico_targetbinaria(df_input['Densidad'], varObjBin1, 'Densidad')
...
# %% Veo graficamente el efecto de dos variables cualitativas sobre la binaria
mosaico_targetbinaria(df_input['ActividadPpal'], varObjBin2, 'ActividadPpal')
mosaico_targetbinaria(df_input['Densidad'], varObjBin2, 'Densidad')

# Veo graficamente el efecto de dos variables cuantitativas sobre la binaria2
boxplot_targetbinaria(df_input['UnemployLess25_Ptge'], varObjBin2, nombre_ejeX='Izquierda',
                      nombre_ejeY='UnemployLess25_Ptge')
boxplot_targetbinaria(df_input['AgricultureUnemploymentPtge'], varObjBin2, nombre_ejeX='Izquierda',
                      nombre_ejeY='AgricultureUnemploymentPtge')
...
# %% Veo graficamente el efecto de dos variables cualitativas sobre la binaria
mosaico_targetbinaria(df_input['ActividadPpal'], varObjBin3, 'ActividadPpal')
mosaico_targetbinaria(df_input['Densidad'], varObjBin3, 'Densidad')
...
# %% Busco las mejores transformaciones para las variables numericas con respecto a los dos tipos de variables
varObjBin1f = pd.Categorical(varObjBin1).codes
varObjBin2f = pd.Categorical(varObjBin2).codes
varObjBin3f = pd.Categorical(varObjBin3).codes

input_bin1 = pd.concat([df_input, Transf_Auto(df_input[numericas], varObjBin1f)], axis=1)
input_bin2 = pd.concat([df_input, Transf_Auto(df_input[numericas], varObjBin2f)], axis=1)
input_bin3 = pd.concat([df_input, Transf_Auto(df_input[numericas], varObjBin3f)], axis=1)

# %%  Creamos conjuntos de datos que contengan las variables explicativas y una de las variables objetivo y los
# guardamos
